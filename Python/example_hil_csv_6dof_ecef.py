...
import time
import codecs
FILE_PATH = 'circle_1000Hz_6dof_ecef.csv'
...
def readRow(row):
  ...
  # Each row has [Timestamp (ms), X (m), Y (m), Z (m), Yaw (deg), Pitch (deg), Roll (deg)] as string
  timestamp = int(row[0])
  x = float(row[1])
  y = float(row[2])
  z = float(row[3])
  ...
  while not eof:
    ...
    while not eof:
      if csvTimestamp >= targetCsvTimeStamp:
        break
      if ecef is not None:
        sim.pushEcefNed(csvTimestamp, ecef, attitude)
        pushCount += 1
        ecef = None
      try: 
        row = next(reader)
        csvTimestamp, ecef, attitude = readRow(row)
