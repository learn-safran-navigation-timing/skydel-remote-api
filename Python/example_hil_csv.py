...
# You must execute "example_generate_circle_csv.py" before to generate the csv
...
def readRow(row):
  ...
  # Each row has [timestamp (ms), Lat (deg), Lon (deg), Alt (m)] as string
  timestamp = int(row[0])
  lat = float(row[1])
  lon = float(row[2])
  alt = float(row[3])
  ...
  while not eof:
    ...
    while not eof:
      if csvTimestamp >= targetCsvTimeStamp:
        break
      if lla is not None:
        sim.pushLla(csvTimestamp, lla)
        pushCount += 1
        lla = None
      try: 
        row = next(reader)
        csvTimestamp, lla = readRow(row)
